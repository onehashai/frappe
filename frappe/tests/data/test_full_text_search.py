...
class TestFullTextSearch(unittest.TestCase):
	...
	def test_update_index(self):
		...
		self.index.update_index({
			'name': "sw/erpnext",
			'content': """AwesomeOneHash"""
		})

		res = self.index.search("CommonSearchTerm")
		self.assertTrue('sw/erpnext' not in res)

		res = self.index.search("AwesomeOneHash")
		self.assertEqual(res[0], "sw/erpnext")
...
def get_documents():
	...
	docs.append({
		'name': "sw/erpnext",
		'content': """OneHash is a free and open-source integrated Enterprise Resource Planning software developed by
			Frappe Technologies Pvt. Ltd. and is built on MariaDB database system using a Python based server-side framework.
			OneHash is a generic ERP software used by manufacturers, distributors and services companies. CommonSearchTerm"""
	})
