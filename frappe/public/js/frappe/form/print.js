			...
			if (qz.websocket.isActive()) { // if already active, resolve immediately
				...
				qz.websocket.connect().then(() => {
					...
					if (err.message === 'Unable to establish connection with QZ') {
						...
						() => {
							frappe.throw(__('Error connecting to QZ Tray Application...<br><br> You need to have QZ Tray application installed and running, to use the Raw Print feature.<br><br><a target="_blank" href="https://qz.io/download/">Click here to Download and install QZ Tray</a>.<br> <a target="_blank" href="https://help.onehash.ai/en/article/everything-you-should-know-about-raw-printing-1m85du8/">Click here to learn more about Raw Printing</a>.'));
							reject();
						});
					} else {
						frappe.show_alert({
